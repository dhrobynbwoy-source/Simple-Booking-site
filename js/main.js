...
/* ---------- Synchronisation avec la réplique du tableau de bord ---------- */
// Si le simulateur est placé dans un .demo-frame accompagné d'une réplique
// de tableau de bord (page Démonstration), on met à jour les compteurs et
// la liste des demandes pour montrer, en direct, l'arrivée de la réservation.
function syncDashboard(root, state) {
  var frame = root.closest(".demo-frame");
  if (!frame) return;

  var list = frame.querySelector("[data-dash-list]");
  if (list) {
    var row = document.createElement("div");
    row.className = "dash-row";
    row.innerHTML =
      '<div><span class="who">' + (state.name || "Client") + '</span><span class="service">' + (state.service || "") + " · " + (state.slot || "") + "</span></div>" +
      '<span class="pill pending">En attente</span>';
    list.prepend(row);
  }

  bumpMetric(frame, "pending", 1);
  bumpMetric(frame, "clients", 1);
}

function bumpMetric(frame, key, delta) {
  var el = frame.querySelector('[data-metric="' + key + '"]');
  if (!el) return;
...
function resetDashboard(root) {
  ...
  var list = frame.querySelector("[data-dash-list]");
  if (list) {
    list.querySelectorAll(".dash-row").forEach(function (row, i) {
      if (i < 2) return; // garde les deux exemples de départ
      row.remove();
    });
  }

  var pendingEl = frame.querySelector('[data-metric="pending"]');
  var clientsEl = frame.querySelector('[data-metric="clients"]');
  if (pendingEl) pendingEl.textContent = "0";
  if (clientsEl) clientsEl.textContent = "12";
}
